...
### Chart B
Scatterplot of order number and days since last order, with different colors for different departments
```{r}
instacart %>% 
...
```{r}
instacart %>% 
  group_by(add_to_cart_order, reordered) %>% 
  plot_ly(y = ~add_to_cart_order, color = ~reordered, type = "box",
          colors = "Set2")
```
